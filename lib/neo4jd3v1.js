...
function Neo4jD3(_selector, _options) {//创建 Neo4jD3 图形对象
    ...
    function appendNode() {
        return node.enter()//创建一个新的节点
                   ...
                   .on('click', function(d) {
                       // d.fx = d.fy = null;//表示释放节点的固定位置，允许节点重新参与到力导向布局的计算中。
                       //  //如果 options.onNodeClick 是一个函数，则会调用它
...
n.each(function(d) {
    ...
    function appendOutlineToNode(node) {
        console.log("执行appendOutlineToNode()函数,节点信息："  )
        console.log(node.data()[0])
  const g = node.append('g') // 创建一个<g>元素，用于容纳图表元素
    .attr('class', 'outline');

var subcolor =["#99d8c9","#238b45","#005824"];
var bigcolor = ["#fc9272", "#feb24c", "#78c679", "#74a9cf", "#8c6bb1", "#df65b0", "#737373", "#969696", "#bdbdbd", "#d9d9d9"];
...
console.log(bigtopicdata)
  ...
  g1.selectAll('path')
    ...
    .append('path')
    .attr('d', arc1)
    .attr('fill',  d => subcolor[d.data.name])
    .attr('stroke', 'white')
    .attr('stroke-width', 1);
